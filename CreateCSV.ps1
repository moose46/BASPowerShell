...
Set-Variable -Name TABLE_NAME -Value 'ATTACHMENT'
Set-Variable -Name EXPORT_HEADER_FILE_NAME  -Value 'ATTACHMENT_HEADERS.csv'
Set-Variable -Name EXPORT_TXT_FILENAME -Value 'ATTACHMENT.txt'
Set-Variable -Name EXPORT_PATH -Value 'C:\Users\me\Source\Repos\BabbleFishV4\BabbleFishV3\powershell\'
Set-Variable -Name EXPORT_DATA_FILE_NAME -Value "ATTACHMENT_EXPORT.csv"
cd $EXPORT_PATH
...
#sqlcmd --% -S CRM -E  -Q "select ATTACHMENT from babblefish.information_schema.tables where table_type = 'BASE TABLE'"
...
#sqlcmd  -E  -Q "DECLARE @colnames VARCHAR(max);select @colnames = COALESCE(@colnames + ',', '') + column_name from babblefish.information_schema.COLUMNS where table_name = 'ATTACHMENT';" > ATTACHMENT_HEADERS.csv
